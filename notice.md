...
## 2026-07-23 · v1.0 发布前最终调整

- 汉化管理顶部的“重新检测”“字体包目录”“游戏字体目录”“本地设置目录”由右侧纵向排列调整为底部横向四列，统一按钮宽度并释放状态与路径信息的横向显示空间。
- 本次调整作为公开发行版 1.0 的最后一次代码更新；完成 x86 与 x64 手动打包并上传 GitHub 后，后续开发版本进入 1.1。
- 距离当晚 DLC 发布已非常接近，发行后首先核对实际游戏版本、汉化文本兼容性和在线清单标签，再决定 1.1 的首批适配内容。
...
- 截至 2026-07-22 晚间 DLC 发售前，汉化文本在线更新、国内镜像/官方线路切换、版本标签、安装状态和作者/分享者弹窗均已完成实际运行测试。
- 用户测试通过后补充调整了主窗口“离线和联机”页面说明，明确自定义账户仅适用于学习版。
- 当前代码进入发行前同步阶段；DLC 上线后优先复核游戏版本号、汉化包兼容标签、文本目录结构及在线清单内容。

### 作者与分享者署名

- 在线与手动汉化文本包支持从同名 `.package.json` 读取 `authorName` 和 `sharerName`。
- 安装确认弹窗移除固定作者文本，统一按照“作者 → 分享者 → 未知作者”的顺序显示。
- 内置 `set有简述版v1.10` 保留作者 `QQ群1070483622@橙子`；“词缀职业显示增强”显示分享者 `发光的洛伦兹`。
- 下载服务会把远端清单中的署名字段写入本地 `.package.json`，离线扫描后仍可正常显示。
